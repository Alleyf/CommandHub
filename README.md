...
| 重复文件扫描结果 | ![重复文件扫描](docs/media/screenshots/duplicates-result.png) |
| 文件过期提醒结果 | ![文件过期提醒](docs/media/screenshots/stale-files-result.png) |
...
| 类型 | 推荐方式 | 产出路径 |
| --- | --- | --- |
| 截图 | Windows `Win + Shift + S` / macOS `Shift + Command + 4` | `docs/media/screenshots/*.png` |
| 录屏（系统） | Windows Xbox Game Bar / macOS QuickTime | `docs/media/videos/*.mp4` |
| 录屏（FFmpeg） | 命令行录屏，便于自动化批量产出 | `docs/media/videos/*.mp4` |

FFmpeg 录屏示例（Windows）：

```powershell
ffmpeg -f gdigrab -framerate 30 -i desktop -c:v libx264 -preset veryfast -crf 23 docs/media/videos/command-flow.mp4
```

FFmpeg 裁剪与压缩示例：

```powershell
ffmpeg -i docs/media/videos/command-flow.mp4 -vf "scale=1600:-2,fps=24" -c:v libx264 -crf 24 -preset medium docs/media/videos/command-flow.web.mp4
```
